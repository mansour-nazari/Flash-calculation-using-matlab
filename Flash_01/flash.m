...
while tol > 1e-12 
        IN=IN+1; % updating indactor
        nv=rashford_rice(K,z',nv_old); % rashford-rice equation
        nv_old = nv; 
        x = z./(1+nv_old*(K-1)); % x calc
        y = K.*z./(1+nv_old*(K-1)); % y calc
        [Phi, Z, Rho] = EOS(P,T,data,binary,model,x,y); %EOS
        K = Phi{1}./Phi{2}; % NEW k
        tol = sum((1-((x.*Phi{1})./(y.*Phi{2}))).^2); % erreo calc
        error(IN)=tol;
end
